...
(function(){
  ...
  function styleAndBindTrigger(el, set, mode, zTrigger){
    ...
    el.classList.add('hm-sm-trigger', `hm-sm-trigger--${pos}`, `hm-sm-trigger--${set._index}`);
    ...
    if (shape === 'circle') {
      ...
      /* HM default rect size */
      try { el.style.height = '50px'; el.style.width = '250px'; } catch(e){}
 // 角丸と基本パディング

// --- 追加デザイン適用（セットごと） ---
try { // 例外を避けるためのガード
  const b = set.button || {}; // ボタン設定の参照
  // 角丸：円形以外のときのみ上書き（円形は50%を尊重）
  if (shape !== 'circle' && parseInt(b.border_radius_px,10) > 0) { el.style.borderRadius = parseInt(b.border_radius_px,10) + 'px'; } // 角丸
  // 枠線：値があれば適用、スタイル未指定時はsolidを既定、width=0ならborderを消す
  const bw = parseInt(b.border_width_px,10); // 枠線太さ
  const bs = (b.border_style || '').trim(); // スタイル
  const bc = (b.border_color || '').trim(); // カラー
  if (!isNaN(bw) && bw > 0) { // 太さが指定されている場合
    el.style.borderWidth = bw + 'px'; // 太さ
    el.style.borderStyle = (bs || 'solid'); // スタイル
    if (bc) { el.style.borderColor = bc; } // カラー
  } else if (bs === 'none') { // 明示的にnoneが指定された場合
    el.style.border = 'none'; // 枠線を消す
  }
  // フォント太さ
  if (b.font_weight) { el.style.fontWeight = String(b.font_weight); } // 文字の太さ
  // レター間隔
  if (!isNaN(parseFloat(b.letter_spacing_px)) && parseFloat(b.letter_spacing_px) !== 0) { el.style.letterSpacing = parseFloat(b.letter_spacing_px) + 'px'; } // letter-spacing
} catch(e){} // エラーは握り潰す

      try { // 長方形サイズ指定があれば適用
        const G = (window.HM_SM_DATA && HM_SM_DATA.settings) ? HM_SM_DATA.settings : {}; // グローバル設定
        const wRect = (mode==='sp') ? (set.button.w_rect_px_removed||G.rect_w_px||0) : (mode==='tab') ? (set.button.w_rect_px_removed||G.rect_w_px_tab||0) : (set.button.w_rect_px_removed||G.rect_w_px_pc||0); // 幅
        const hRect = (mode==='sp') ? (set.button.h_rect_px_removed||G.rect_h_px||0) : (mode==='tab') ? (set.button.h_rect_px_removed||G.rect_h_px_tab||0) : (set.button.h_rect_px_removed||G.rect_h_px_pc||0); // 高さ
        if (parseInt(wRect,10)>0) { el.style.width = parseInt(wRect,10) + 'px'; } // 幅適用
        if (parseInt(hRect,10)>0) { el.style.height = parseInt(hRect,10) + 'px'; el.style.lineHeight = parseInt(hRect,10) + 'px'; } // 高さ/行高適用
      } catch(e){} // 例外は無視
    }
  ...
  function openModalForSet(set){
    ...
    const bottomBtn = root.querySelector('.hm-sm__actions .hm-sm__btn'); // 下部クローズボタンを取得
    ...
    if (set.modal.max_width_px && parseInt(set.modal.max_width_px,10)>0) { dialog.style.maxWidth = parseInt(set.modal.max_width_px,10) + 'px'; } else { dialog.style.maxWidth = ''; } // 最大幅を適用（0/未設定は既定値に戻す）
    if (set.modal.min_width_px && parseInt(set.modal.min_width_px,10)>0) { dialog.style.minWidth = parseInt(set.modal.min_width_px,10) + 'px'; } else { dialog.style.minWidth = ''; } // 最小幅を適用（0/未設定は解除）
    if (set.modal.min_height_px && parseInt(set.modal.min_height_px,10)>0) { content.style.minHeight = parseInt(set.modal.min_height_px,10) + 'px'; } else { content.style.minHeight = ''; } // 最小高さを適用（0/未設定は解除）
    ...
    cloneContentForSet(set, function(clone){
      if (clone) {
      // cleanup_remove_selectors: removed (feature deprecated) – no-op for backward compatibility
if (clone) { content.appendChild(clone); var __hmSmClonedRef = clone; }
    }
      ...
      targets.forEach((a)=>{
        ...
        a.style.color = '#fff';
        // Apply modal_button styling (modal-specific only; no fallback to trigger). Defaults are black.
        const mbtn = set.modal_button || {};
        const present = v => !(v === undefined || v === null || (typeof v === 'string' && v.trim() === ''));
        const hex = (c)=> (typeof c === 'string' && c.startsWith('#') ? c : (typeof c === 'string' ? c : '#000000'));
        const bgHex = present(mbtn.bg_color) ? mbtn.bg_color : '#000000';
        const bgOpacity = present(mbtn.bg_opacity) ? mbtn.bg_opacity : 1;
        const textHex = present(mbtn.text_color) ? mbtn.text_color : '#FFFFFF';
        const borderHex = present(mbtn.border_color) ? mbtn.border_color : '#000000';
        const borderOpacity = present(mbtn.border_opacity) ? mbtn.border_opacity : 1;
        const borderW = present(mbtn.border_width_px) ? mbtn.border_width_px : 1;
        a.style.background = hexToRgba(hex(bgHex), bgOpacity);
        a.style.color = hex(textHex);
        a.style.border = (parseInt(borderW,10)||0) + 'px solid ' + hexToRgba(hex(borderHex), borderOpacity);
        a.style.transition = 'all ' + (present(mbtn.hover_transition_sec) ? mbtn.hover_transition_sec : 0.4) + 's';
        // Hover styles (modal-only)
        const hoverBgHex = present(mbtn.hover_bg_color) ? mbtn.hover_bg_color : bgHex;
        const hoverBgOpacity = present(mbtn.hover_bg_opacity) ? mbtn.hover_bg_opacity : 0.8;
        const hoverTextHex = present(mbtn.hover_text_color) ? mbtn.hover_text_color : textHex;
        const hoverBg = hexToRgba(hex(hoverBgHex), hoverBgOpacity);
        const normalBg = hexToRgba(hex(bgHex), bgOpacity);
        const normalText = hex(textHex);
        a.addEventListener('mouseenter', () => { a.style.background = hoverBg; a.style.color = hex(hoverTextHex); });
        a.addEventListener('mouseleave', () => { a.style.background = normalBg; a.style.color = normalText; });
        a.addEventListener('focus', () => { a.style.background = hoverBg; a.style.color = hex(hoverTextHex); });
        a.addEventListener('blur', () => { a.style.background = normalBg; a.style.color = normalText; });
        /*__MODAL_BUTTON_STYLE__*/
      ...
      if (targets.length > 0) { if (typeof __hmSmClonedRef !== 'undefined' && __hmSmClonedRef) { try { __hmSmClonedRef.style.display = 'none'; } catch(e){} } content.appendChild(grid); }
    });
